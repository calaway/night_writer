...
class NightWriterTest < Minitest::Test
  ...
  def test_it_can_translate_row_0
    ...
    input = "a"
    assert_equal "0.", nightwriter1.braille_string(input, 0)

    message_2 = "helloworld"
    assert_equal "0.0.0.0.0..00.0.0.00", nightwriter1.braille_string(message_2, 0)
  end

  def test_it_can_return_same_message_in_all_rows
    nightwriter1 = NightWriter.new
    input = "abc"
    assert_equal ["0.0.00", "..0...", "......"], nightwriter1.three_row_translation(input)
  end
...
'00.000.000'] , nightwriter1.braille_translate_with_line_breaks(plaintext_message)
  ...
  def test_parse_plaintext_to_add_number_symbol
    ...
    assert_equal "#1 a" , nightwriter1.parse_plaintext_for_numbers(plaintext_message)
    nightwriter1 = NightWriter.new
    plaintext_message = " 1234567890!',-.?abcdefghijklmnopqrstuvwxyzABCDEFGHI2345JKLMNOP3456QRSTUVWX1YZ"
    assert_equal " #1234567890 !',-.?abcdefghijklmnopqrstuvwxyzABCDEFGHI#2345 JKLMNOP#3456 QRSTUVWX#1 YZ" , nightwriter1.parse_plaintext_for_numbers(plaintext_message)
  end
